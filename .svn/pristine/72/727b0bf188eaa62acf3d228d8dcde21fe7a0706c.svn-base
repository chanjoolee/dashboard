 ...
 */
@Controller
public class CodeComplexTrendsJsonController {

    private static final Logger logger = LoggerFactory.getLogger(CodeComplexTrendsJsonController.class);
    ...
    @RequestMapping(value = "/codeComplexTrendsJson")
    public ModelAndView codeComplexTrendsJson(HttpServletRequest request,@RequestParam Map<String,String> parameter,Locale locale, Model model) throws IOException {
    	...
    	java.io.File codeComplexDir = new java.io.File(curDir.getParentFile().toURI().getPath()+"/codeComplexity");
    	...
    	try{
        	...
        	java.io.File[] dirs = codeComplexDir.listFiles();
...
//            mav.addObject("pngPath",pngPath );
    		...
    		String imagePath =  "";
//    		imagePath =  "http://"+request.getServerName()+":"+ request.getServerPort()+ "/codeComplexity/1.RiskyGroup.png";
    		
    		
    		imagePath =  "http://"+request.getServerName()+":"+ request.getServerPort();
    		imagePath += "/codeComplexity/"+lastDir.getName();
    		//imagePath += "/"+parameter.get("pjt_name").toString();
    		String project = parameter.get("pjt_name").toString().split(" ")[0];
    		imagePath += "/"+project;
    		//imagePath += "/"+parameter.get("pjt_name").toString()+"-risk_group.png";
    		imagePath += "/"+project+"-risk_group.png";
            mav.addObject("imagePath",imagePath );
            
    	}catch(Exception e){
    		 mav.addObject("errMessage",e.getMessage() );
    		 mav.addObject("curDir",curDir.getAbsolutePath());
    		 mav.addObject("regressionDir",codeComplexDir.getAbsolutePath());
    	}
